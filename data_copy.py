...
def default_loader(id, root, round_folder, split):
    if split == 'train':

        im_path = os.path.join(root,'images','{}_sat.jpg').format(id)
        m_path = os.path.join(round_folder,'gt','{}_mask.png').format(id)
        ske_gt = os.path.join(round_folder,'gt_sk','{}_mask.png').format(id)
    ...
    #   print(m_path)
        ...
        img = cv2.resize(img, (0,0), fx=scal, fy=scal, interpolation=cv2.INTER_LINEAR)
        mask = cv2.resize(mask, (0,0), fx=scal, fy=scal, interpolation=cv2.INTER_NEAREST)
        d_mask = cv2.resize(d_mask, (0,0), fx=scal, fy=scal, interpolation=cv2.INTER_NEAREST)
...
class ImageFolder_Target(data.Dataset):

    def __init__(self, trainlist, root, round_folder, split):
        self.ids = trainlist
    ...
    def __getitem__(self, index):
        id = self.ids[index]
        # try:
        img, mask, d_mask = self.loader(id, self.root, self.round_folder, self.split)
        # print(img)
